...
def run_feature_selection(
    ...
    #     if param_values["type"] == "list":
    #         param_grid[param] = param_values["values"]
    #     elif param_values["type"] == "np.arange":
    #         param_grid[param] = np.arange(param_values["start"], param_values["stop"], param_values["step"])
    #     elif param_values["type"] == "np.linspace":
    #         param_grid[param] = np.linspace(param_values["start"], param_values["stop"], param_values["num"])
    #     elif param_values["type"] == "np.logspace":
    #         param_grid[param] = np.logspace(param_values["start"], param_values["stop"], param_values["num"])
    #     else:
    #         raise ValueError(f"Parameter type {param_values['type']} not supported.")
    ...
    cv_strategy = config["feature_selection"]["cv_strategy"]
    if cv_strategy not in ["stratifiedkfold", "kfold"]:
        raise ValueError(f"Cross-validation strategy {cv_strategy} not supported.")
    if cv_strategy == "stratifiedkfold":
        n_splits = config["feature_selection"]["cv"]
        if n_splits < 2:
            raise ValueError(f"Number of splits (cv) must be at least 2.")
    ...
    if hyperparameter_optimization_method == "gridsearch":
        gridsearch = GridSearchCV(pipeline, param_grid=param_grid, cv=cv_strategy, verbose=verbose, scoring=scoring, refit=refit, return_train_score=True)
    ...
    # save the gridsearch object
    dump(gridsearch, output_path_gridsearch)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output_path_gridsearch",
        type=str,
        help="The output path to save the gridsearch object.",
        required=False,
    )
    parser.add_argument(
        "--output_path_model",
    ...
    # print("Top memory usage differences:")
    # for stat in top_stats[:10]:  # Print top 10 lines with the highest memory usage 
    #     print(stat)
